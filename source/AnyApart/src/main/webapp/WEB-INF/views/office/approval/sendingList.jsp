...
<form id="viewForm" method="get" action="${cPath }/office/approval/draftView.do">
	...
	<input type="hidden" name="flag" value="sending" />
</form>
...
<div class="container">
	 ...
	 <div class="row">
		 <button onclick = "location.href = '${pageContext.request.contextPath }/office/approval/draftForm.do'" 
				 class="btn btn-dark" id="insertBtn">새 결재</button>
	 </div>
	<table class="table text-center"> 
	  ...
	  <tbody id="listBody">
	   <c:set var="sendingList" value="${pagingVO.dataList }"/>
		  	<c:if test="${not empty sendingList }">
		  		<c:forEach items="${sendingList }" var="sending">
		  			<tr data-draft-id='${sending.draftId }'>
		  				<td>${sending.draftId }</td>
		  				<td class="text-left">${sending.draftTitle }</td>
		  				<td>${sending.draftWriter }</td>
		  				<td>
		  					<c:if test="${sending.approval.appStatus eq '기안취소' }">
			  					<span class="badge badge-secondary">${sending.approval.appStatus }</span>
			  				</c:if>
			  				<c:if test="${sending.approval.appStatus eq '대기중' }">
			  					<span class="badge badge-primary">${sending.approval.appStatus }</span>
			  				</c:if>
			  				<c:if test="${sending.approval.appStatus eq '반려' }">
			  					<span class="badge badge-warning text-dark">${sending.approval.appStatus }</span>
			  				</c:if>
			  				<c:if test="${sending.approval.appStatus eq '결재중' }">
			  					<span class="badge badge-info">${sending.approval.appStatus }</span>
			  				</c:if>
			  				<c:if test="${sending.approval.appStatus eq '결재완료' }">
			  					<span class="badge badge-dark">${sending.approval.appStatus }</span>
			  				</c:if>
		  				</td>
		  				<td>${sending.draftDate }</td>
				    <tr>
		  		</c:forEach>
			</c:if>
