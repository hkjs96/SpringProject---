...
<div class="container">
	...
	<div class="d-flex justify-content-center">
	 <c:if test="${board.boWriterId eq authMember.memId}">
		<form id="updateForm" action="${cPath }/office/website/officeQna/officeQnaUpdate.do" method="GET">
		...
		<form id="deleteForm" action="${cPath }/office/website/officeQna/officeQnaDelete.do" method="POST">
			<input type="hidden" class="form-control" name="boNo" value="${board.boNo }"/>
		...
		<c:url value="/office/website/officeQna/officeQnaAnswer.do" var="answerURL">
			<c:param name="boNo" value="${board.boNo }"/>
		</c:url>
		<c:if test="${board.boDepth eq 1}">
			<input type="button" value="답변" class="btn btn-success" id="answerBtn">
		</c:if>
		<form id="listForm" action="${cPath }/office/website/officeQna/officeQnaList.do" method="GET">
			<input type="hidden" class="form-control" name="page" value="${param.page }"/>
			<input type="hidden" class="form-control" name="searchType" value="${param.searchType }"/>
			<input type="hidden" class="form-control" name="searchWord" value="${param.searchWord }"/>
...
<script type="text/javascript">
	$(function(){
		$.ajax({
			url:"${cPath }/office/website/officeQna/repChk.do"
