...
<div> 
	<button type="button" class="btn btn-primary" style='margin:5pt;' data-toggle="modal" data-target="#insertModal">공동 검침량 단건 등록</button>
...
<!----------------------------- 검색 --------------------------------->
<div class="container">
	<div class="col-md-9" style="border-style: outset; border-radius: 8px;margin-left: 10%;">
		<div class="row g-0">
		...
		<div class="card-body">
			<div class="row">
				...
				<div class="col-md-10">
					<input type="text" name="startYear" class="form-control col-md-2" value="${pagingVO.searchDetail.startYear }" onkeypress="return onlyNumber(event)" maxLength="4">
		      		<span>년&nbsp;</span>
					<input type="text" name="startMonth" class="form-control col-md-1" value="${pagingVO.searchDetail.startMonth }" onkeypress="return onlyNumber(event)" maxLength="4">
		      		<span>월&nbsp;</span>&nbsp; ~ &nbsp;
					<input type="text" name="endYear" class="form-control col-md-2" value="${pagingVO.searchDetail.endYear }" onkeypress="return onlyNumber(event)" maxLength="4">
		      		<span>년&nbsp;</span>
					<input type="text" name="endMonth" class="form-control col-md-1" value="${pagingVO.searchDetail.endMonth }" onkeypress="return onlyNumber(event)" maxLength="4">
		      		<span>월&nbsp;</span>&nbsp;&nbsp;&nbsp;
		      		<button type="button" class="btn btn-dark" id="sixMonthBtn">6개월</button>
		      		<button type="button" class="btn btn-dark" id="oneYearBtn">1년</button>
			 	</div>
			</div>
			<br>
			<div class="row">
				<div class="col-md-2">검침항목	</div>
		      	<div class="col-md-5">
					<select name="meter" class="custom-select col-md-5 searchSelect">
						<option value>항목선택</option>
						<option value="COMM_HEAT" ${'COMM_HEAT' eq pagingVO.searchDetail.meter?"selected":""}>난방</option>
						<option value="COMM_HOTWATER" ${'COMM_HOTWATER' eq pagingVO.searchDetail.meter?"selected":""}>급탕</option>
						<option value="COMM_WATER" ${"COMM_WATER" eq pagingVO.searchDetail.meter?"selected":""}>수도</option>
						<option value="COMM_ELEC" ${"COMM_ELEC" eq pagingVO.searchDetail.meter?"selected":""}>전기</option>
					</select> &nbsp;&nbsp;
					<select name="sort" class="custom-select col-md-5 searchSelect">
...
<div id="explainDiv">
	...
	<br>
	<span>* [${apart.aptName }]의 난방방식은 [${apart.codeName }]입니다.</span><br>
</div>
...
<div style="width: 100%;height: 550px;overflow: auto;">
	...
	<%-- 						</c:when> --%>
							<c:when test="${cmVO.commFlag eq 'N'}">
								<button type="button" id="updateBtn" class="btn btn-warning updateBtn" data-commno="${cmVO.commNo }">수정</button>
								<button type="button" id="deleteBtn" class="btn btn-danger deleteBtn" data-commno="${cmVO.commNo }">삭제</button>
							</c:when>
							<c:otherwise>
...
$("#insertForm").validate({
	rules: 
    {
        ...
        ,commHeat:{maxlength: 12, min:0}
        ,commHotwater:{maxlength: 12, min:0}
        ,commWater:{maxlength: 12, min:0}
        ,commElec:{maxlength: 12, min:0}
    }
});
...
$("#oneYearBtn").on("click", function(){
	let startDate = new Date(date.setMonth(date.getMonth()-11));
	let startYear = startDate.getFullYear();
	let startMonth = startDate.getMonth()+1;
	searchForm.find("[name='startYear']").val(startYear);
	searchForm.find("[name='startMonth']").val(startMonth);
