...
<div class="container d-flex justify-content-center" >
	<div class="col-md-6  " style="border-style:outset;border-radius: 8px;">
		<div class="row g-0">
			<div class="mt-2 mb-2 d-flex justify-content-between col-sm-12">
				<div class="d-flex justify-content-center">
					<i class="fas fa-money-check fa-2x" style="margin-left: 10px;"></i>
				</div>
				<span class="costTitle">&nbsp;&nbsp;<strong>${commMeter.commYear }년 ${commMeter.commMonth }월 관리비</strong></span>
				<c:if test="${commMeter.commFlag eq 'Y' }">
					<div class="d-flex justify-content-end"><button class="btn btn-danger btn-lg" disabled>부과마감</button></div>
				</c:if>
				<c:if test="${commMeter.commFlag eq 'N' }">
					<div class="d-flex justify-content-end"><button class="btn btn-danger btn-lg" data-toggle="modal" data-target="#costInsertAuthModal">부과처리</button></div>
				</c:if>
			</div>
		</div>
	</div>
		...
		<div class="col-sm-12" style="border-style: outset; border-radius: 8px;">
			<div class="row g-0">
				<div class="col-md-12" style="margin-top: 20px;">
					<i class="fas fa-check fa-2x" style="margin-left: 10px; margin-top: 10px;"></i>
					<span>&nbsp;&nbsp;<strong>부과 정보</strong></span>
				</div>
			</div>
			<div class="card-body row">
				<div class="d-flex justify-content-between col-sm-12">
					<div class="col-md-6 mr-2 float-left">
						<canvas id="houseAreaChart"></canvas>
					</div>
					<div class="col-md float-left">	
						<div class="d-flex justify-content-between mb-3">
							<input type="button" value="부과기준표"  class="btn btn-info" onclick="viewCostStandard();"/>
							<span class="reddot justify-content-middle">* 부과처리 후에는 검침량을 수정할 수 없습니다.</span>
						</div>	
						<table class="table table-bordered table-sm table-hover">
							<thead class="thead-dark text-center">
								<tr>
									<th colspan="4">부과기본정보</th>
								</tr>
							</thead>
							<tbody class="thead-light text-center">
								...
								<tr>
									<th>부과년월</th>
									<td colspan="3">${commMeter.commYear }년 ${commMeter.commMonth }월 관리비</td>
								</tr>
								<tr>
									<th>입주 세대수</th>
									<td class="text-right"><fmt:formatNumber type="number" maxFractionDigits="3" value="${house.moveinHouseCnt }" /> 세대</td>
									<th>총세대수</th>
									<td class="text-right"><fmt:formatNumber type="number" maxFractionDigits="3" value="${house.totalHouseCnt }" /> 세대</td>
								</tr>
								<tr>
									<th>입주 면적</th>
									<td class="text-right"><fmt:formatNumber type="number" maxFractionDigits="3" value="${house.moveinHouseArea }" /> ㎡</td>
									<th>총 주거면적</th>
									<td class="text-right"><fmt:formatNumber type="number" maxFractionDigits="3" value="${house.totalHouseArea }" /> ㎡</td>
								</tr>
							</tbody>
						</table>
					</div>
	...
	<div class="card-body row">
		<div class="col-sm-12">
			<div class="justify-content-center">
				<form:form modelAttribute="pagingVO" id="searchForm" method="get">
			    	...
			    	<c:if test="${not empty costList }">
			    		<c:forEach items="${costList }" var="cost" varStatus="vs">
							<tr>
								...
								<c:set var="costCommTotal" value="${cost.houseArea * costCommTotalBySpaceTotal }"/><!-- 세대수 적어서 우선10으로 나눠뒀었는데 그냥 둠 -->
			...
			<form id="costInsertAuthForm" action="${cPath }/office/cost/costInsert.do" method="post">
				...
				<div class="modal-body">
					<table class="table table-bordered">
						<tr>
							<th class="table-secondary text-center" colspan="2">${date } 기준<br>${commMeter.commYear }년 ${commMeter.commMonth }월 관리비 부과처리</th>
						</tr>
						<tr>
							<th class="table-warning text-center" colspan="2">* 부과처리 후에는 검침량 및 관리비를 수정할 수 없습니다.</th>
						</tr>
						<tr>
...
var config = {
	...
	,data: {
		...
		,datasets: [{
			data: [${house.moveinHouseArea}, ${house.totalHouseArea - house.moveinHouseArea}]
			,backgroundColor: [
				  "#FF6384",
				  "#FFCE56"
			]
...
$(function(){
	<c:if test="${commMeter.commFlag eq 'Y' }">
		$('#indvListBody').css("background-image",`url('${cPath}/images/charged2.png')`)
