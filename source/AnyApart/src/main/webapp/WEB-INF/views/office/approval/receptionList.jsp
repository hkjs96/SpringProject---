...
<form id="viewForm" method="get" action="${cPath }/office/approval/draftView.do">
	...
	<input type="hidden" name="flag" value="reception" />
</form>
...
<div class="container">
	...
	<table class="table text-center"> 
	  ...
	  <tbody id="listBody">
	   <c:set var="receptionList" value="${pagingVO.dataList }"/>
		  	<c:if test="${not empty receptionList }">
		  		<c:forEach items="${receptionList }" var="reception">
		  			<tr data-draft-id='${reception.draftId }'>
		  				<td>${reception.draftId }</td>
		  				<td class="text-left">${reception.draftTitle }</td>
		  				<td>${reception.draftWriter }</td>
		  				<td>
			  				<c:if test="${reception.approval.appStatus eq '기안취소' }">
			  					<span class="badge badge-secondary">${reception.approval.appStatus }</span>
			  				</c:if>
			  				<c:if test="${reception.approval.appStatus eq '대기중' }">
			  					<span class="badge badge-primary">${reception.approval.appStatus }</span>
			  				</c:if>
			  				<c:if test="${reception.approval.appStatus eq '반려' }">
			  					<span class="badge badge-warning text-dark">${reception.approval.appStatus }</span>
			  				</c:if>
			  				<c:if test="${reception.approval.appStatus eq '결재중' }">
			  					<span class="badge badge-info">${reception.approval.appStatus }</span>
			  				</c:if>
			  				<c:if test="${reception.approval.appStatus eq '결재완료' }">
			  					<span class="badge badge-dark">${reception.approval.appStatus }</span>
			  				</c:if>
		  				</td>
		  				<td>${reception.draftDate }</td>
				    <tr>
		  		</c:forEach>
			</c:if>
