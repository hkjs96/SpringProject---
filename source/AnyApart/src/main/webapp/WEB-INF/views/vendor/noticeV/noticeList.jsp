...
<div id="container">
	<div class="inner">
		...
		<div class="board-list">
			<div align="right">
				<input class="btn btn-green" type="button" value="새글 등록"
					onclick="location.href='${cPath }/vendor/noticeForm.do'">
			</div>
			<br>
			<table>
				<colgroup>
				...
				<tbody>
					...
					<c:if test="${not empty boardList }">
						<c:forEach items="${boardList }" var="board">
							<tr>
								<td>${board.rnum}</td>
								<c:url value="/vendor/noticeView.do?boNo=${board.boNo }"
									var="viewURL" />
								<td class="subject"><a
								...
								<td>${board.boDate }</td>
								<td>${board.boHit }</td>
								<td><c:url value="/vendor/noticeUpdate.do" var="updateURL">
										<c:param name="boNo" value="${board.boNo }" />
									</c:url> <input class="btn btn-blue" type="button" value="수정"
									onclick="location.href='${updateURL}'">
									<form id="board" action="${cPath }/vendor/noticeDelete.do"
										method="POST">
										<input type="hidden" class="form-control" name="boNo"
											value="${board.boNo }" /> <input type="hidden"
											class="form-control" name="boType" value="${board.boType }" />
										<input type="hidden" class="form-control" name="boDelete"
											value="${board.boDelete }" /> <input class="btn btn-red"
		...
		<form class="form">
			<div id="inputUI" class="row justify-content-center mb-8">
				<div class="col-md-2">
